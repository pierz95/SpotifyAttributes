    ...
    def forward(self, data):
        ...
        for gc_layer, batch_norm in zip(self.conv_layers, self.gc_layer_norm):
            h = gc_layer(h, edge_index,
                         edge_weight=edge_attr)  # TODO:testare sta cosa che ho aggiunto edge_weight senza far girare il modello
            h = self.conv_act(h)
            h = batch_norm(h)
            h = self.dropout(h)
    ...
    for x, y in phi_rho_layers:
        for lr in config['lr']:
            curr_val_loss, curr_train_m, curr_val_m, curr_test_m, curr_hist_train, curr_hist_val, curr_hist_test = train_deepset(
                config,
                df, Xf, yf, train_id, val_id, test_id, x, y, lr, device)
...
def train_deepset(config, df, Xf, yf, train_id, val_id, test_id, phi_layers, rho_layers, lr, device):
    ...
    model_gnn = base_GC_net(in_channels=111, n_gc_hidden_units='TODO: è un int', conv=torch_geometric.nn.GCNConv,
                            n_layer='TODO: è un int', drop_prob=0.4, conv_act=nn.ReLU(), device=None) #TODO
    model = Deepset(phi_layers=phi_layers, rho_layers=rho_layers, device=device)
    model.to(device)
    ...
    for epoch in range(config['max_epochs']):
        ...
        for i, (_, x, l, y) in enumerate(dataloader['train']):
            ...
            # forward
            h_g=model_gnn(data) #DATA=x,attr,edge....
            h_playlist = []
            y_train = []
            ...
            for i, (x, y) in enumerate(zip(h_playlist,
                                           y_train)):  # TODO: occhio che non ci sono più le liste per utente e vanno ricreate in base a quello che fa pier


                y_i = model(x, l) #TODO:occhio alla l che prima non c'era, vanno modificati i cicli for
...
class Deepset(nn.Module):
    def __init__(self, phi_layers, rho_layers, device):
